...
# prints a given Hand
def printHand(hand, title="", pretty=True):
	cards = hand.card1, hand.card2
	printCards(cards, title=title, pretty=pretty)

# print a list of Card objects
def printCards(cards, title="Cards", pretty=True):
	print("")
	print("******** " + title + " *********")
	for card in cards:
		print (card.to_string(pretty=pretty), end=" ")
...
def parseOptionInput(optionInputString):
	if optionInputString.lower() == 'b':
		bet = input("how much")
		return bet
	else:
		return 0
